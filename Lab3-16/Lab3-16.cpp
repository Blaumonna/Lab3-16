...
public:
    ...
    bool Load();                // Функция load может вернуть код ошибки 
...
{
    try {
        m_pImage = new Magick::Image(m_fileName);  // инициализация класса Magic::Image загружает текстуру в память 
    ...
    glGenTextures(1, &m_textureObj);    //Функция генерирует указанное число объектов текстур и помещает их в указатель на массив GLuint
...
static void RenderSceneCB()
{
    ...
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), 0);                  // glVertexAttribPointer для указания позиции координат текстуры в вершинном буфере. Они представлены в виде 2 вещественных числах, что и указано во 2 и 3 параметрах.
...
{
    Vertex Vertices[4] = { Vertex(Vector3f(-1.0f, -1.0f, 0.5773f), Vector2f(0.0f, 0.0f)),   // структура 'Vertex', содержащая так же и координаты текстуры в формате Vector2f.
...
int main(int argc, char** argv)
{
    ...
    pTexture = new Texture(GL_TEXTURE_2D, "C:/png/14.png");    // индексы модулей текстуры
